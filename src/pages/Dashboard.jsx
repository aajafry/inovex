...
export default function Dashboard() {
  ...
  const { data: clients } = useSWR([CLIENTS_URL, authToken], ([CLIENTS_URL, authToken]) => fetcher(CLIENTS_URL, authToken?.access_token))
  const clientsData = clients?.users?.filter((user) => user?.role === "Client");

  // orders info. fetcher
  const { data: orders } = useSWR([ORDERS_URL, authToken], ([ORDERS_URL, authToken]) => fetcher(ORDERS_URL, authToken?.access_token))
  const ordersData = orders?.orders;

  // invoices info. fetcher
  const { data: invoices } = useSWR([INVOICES_URL, authToken], ([INVOICES_URL, authToken]) => fetcher(INVOICES_URL, authToken?.access_token))
  ...
  const { data: tickets, error, isLoading } = useSWR([TICKETS_URL, authToken], ([TICKETS_URL, authToken]) => fetcher(TICKETS_URL, authToken?.access_token))
